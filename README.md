...
If key-based login is not available, save an optional SSH password on the route; KTClaw stores it in the local encrypted secret store and uses a built-in SSH client to execute the remote OpenClaw command. First-stage SFTP upload/download uses that saved SSH password.
