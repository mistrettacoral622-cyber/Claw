...
For speed, SSH Intercom first tries the remote machine's already-running KTClaw/OpenClaw Gateway on `127.0.0.1:18789` from inside the SSH session, sends through `chat.send`, and polls `chat.history` for the assistant reply. If the remote Gateway is not running or cannot be reached, the same SSH command falls back to the bundled `openclaw agent` CLI path.
