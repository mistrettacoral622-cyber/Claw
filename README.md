...
For speed, SSH Intercom first tries the remote machine's already-running KTClaw/OpenClaw Gateway from inside the SSH session. The route stores the remote Gateway port separately from the SSH port and defaults to `18789`; copied KTClaw connection info includes the current local Gateway port so the controlling machine does not have to guess. Normal messages are dispatched through Gateway `chat.send` in the background, the console waits briefly for a fast reply, and then keeps polling `chat.history` so the UI can show a running state instead of blocking on a cold `openclaw agent` CLI turn. If the remote Gateway is not running, not ready yet, or listening on a different port than the route, normal messages fail fast with a diagnostic instead of cold-starting the CLI path. Structured remote tasks still retain CLI fallback where needed for compatibility and capture/tool workflows.
