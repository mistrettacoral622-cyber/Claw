...
For speed, SSH Intercom first tries the remote machine's already-running KTClaw/OpenClaw Gateway on `127.0.0.1:18789` from inside the SSH session. Normal messages are dispatched through Gateway `chat.send` in the background, the console waits briefly for a fast reply, and then keeps polling `chat.history` so the UI can show a running state instead of blocking on a cold `openclaw agent` CLI turn. If the remote Gateway is not running or cannot be reached, the same SSH command falls back to the bundled `openclaw agent` CLI path.
