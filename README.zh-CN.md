...
聊天输入框中的语音输入现在使用内置的本地 Whisper ASR 模型离线转写。打包后的安装包会直接携带模型文件，因此麦克风转文字不需要 provider API key，也不需要用户在首次运行时额外下载模型。
...
截图也是一次远程任务：控制端请求远端 agent 保存 PNG artifact 并返回路径。这不是连续屏幕共享。
