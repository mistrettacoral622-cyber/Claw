...
为了提速，SSH Intercom 会先在 SSH 会话内部调用远端机器已运行的 KTClaw/OpenClaw Gateway。路由会把远端 Gateway 端口和 SSH 端口分开保存，默认是 `18789`；被控端复制出来的 KTClaw 连接信息会带上当前本机 Gateway 端口，控制端不需要猜。普通消息会通过 Gateway `chat.send` 后台派发，控制台只短暂等待快速回复，随后继续轮询 `chat.history`，让 UI 展示“远端生成中”的状态，而不是阻塞等待一次冷启动的 `openclaw agent` CLI 回合。如果远端 Gateway 没启动、还没 ready，或监听端口和路由配置不一致，普通消息会快速返回诊断，不再冷启动 CLI 路径；结构化远程任务仍保留必要的 CLI fallback，用于兼容和截图/摄像头等工具链兜底。
