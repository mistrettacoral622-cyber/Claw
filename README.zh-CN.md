...
为了提速，SSH Intercom 会先在 SSH 会话内部调用远端机器已运行的 KTClaw/OpenClaw Gateway（`127.0.0.1:18789`）。普通消息会通过 Gateway `chat.send` 后台派发，控制台只短暂等待快速回复，随后继续轮询 `chat.history`，让 UI 展示“远端生成中”的状态，而不是阻塞等待一次冷启动的 `openclaw agent` CLI 回合。如果远端 Gateway 未运行或不可达，同一条 SSH 命令才会回退到内置 `openclaw agent` CLI 路径。
