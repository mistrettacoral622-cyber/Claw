...
### 远程实例与 A2A 分享
KTClaw 可以通过 A2A Agent Card URL 连接其他 KTClaw/OpenClaw 实例，也可以把本机桌面暴露为可信协作者可访问的 inbound A2A endpoint。
在 **设置 -> 远程实例** 中，可以复制 **我的 Agent Card URL**、开启 inbound、选择 Gateway 仅本机可达或同 Wi-Fi/局域网可达、生成或撤销访问 key，并复制给别人使用的 `Authorization: Bearer ...` header。局域网分享会写入 `gateway.bind: "lan"`，但仍需要 Windows 防火墙和路由器/网络放行 Gateway 端口。公网访问不会自动开启；如果要跨公网访问，请使用 Tailscale Serve/Funnel、反向代理、隧道或明确的端口转发。
