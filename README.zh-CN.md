...
截图和拍照都是优化过的一次性远程任务 action：截图会优先走直接 SSH 截图工具；拍照会先请求远端正在运行的 KTClaw 桌面客户端复用聊天里的摄像头拍照流程，再回退到 `imagesnap`、`fswebcam` 或 `ffmpeg` 等平台工具。它们都会以图片 artifact 返回，不是实时桌面、摄像头或屏幕共享流。
