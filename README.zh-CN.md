...
为了提速，SSH Intercom 会先在 SSH 会话内部调用远端机器已运行的 KTClaw/OpenClaw Gateway（`127.0.0.1:18789`），通过 `chat.send` 发送并轮询 `chat.history` 获取 assistant 回复；如果远端 Gateway 未运行或不可达，同一条 SSH 命令会回退到内置 `openclaw agent` CLI 路径。
