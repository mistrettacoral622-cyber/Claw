...
### 远程 Intercom
在 **设置 -> 远程实例** 中配置最小可用的 SSH Intercom 链路，用于跨机器操控 agent。Windows 侧 KTClaw 可以保存 Linux 主机、SSH 用户/端口、目标 agent、session id 和远端 OpenClaw 命令，然后发送一条消息，在 Linux 主机上执行 `openclaw agent --agent <id> --session-id intercom --message ...`。
同一个设置页也会展示本机可分享给别人的 SSH Intercom 配置，包括 host、用户、端口、agent id、session id 和路由示例，另一台 KTClaw 可以直接照着添加本机为可控实例。Windows 上 KTClaw 还会重新生成 `~/.openclaw/gateway.cmd`，让 Gateway 通过应用内置的 Electron Node runtime 运行，不再依赖机器额外安装系统 Node。
发送后，远程实例控制台会用主聊天页同样的对话气泡展示远端回复；命令退出码、stdout、stderr 和耗时仍保留在运行详情里，方便排查 SSH 或远端 OpenClaw 命令问题。
SSH Intercom 会以非交互模式运行：请先配置 SSH key 免密登录；如果需要输入密码、首次 host key 确认，或远端命令一直不退出，界面会直接显示错误而不是长时间卡住。
如果暂时没有免密登录，也可以在路由里保存可选 SSH 密码；KTClaw 会把密码放在本机加密 secret store 中，并使用内置 SSH 客户端执行远端 OpenClaw 命令。
A2A Agent Card 路由仍保留在后端，方便后续继续接入；当前设置面板先隐藏这块，让第一条可用路径聚焦在 Windows 到 Linux 的 SSH 操控。NATS/JetStream 预留给后续可靠队列 transport。
