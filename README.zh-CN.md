...
同一个设置页会检查本机是否已经可以被另一台 KTClaw 控制。点击 **准备这台机器** 可以发起系统授权的 OpenSSH 安装/启动/防火墙放行流程，然后展示另一台 KTClaw 需要填写的 host、用户、端口、agent id、session id 和路由示例。Windows 上 KTClaw 还会重新生成 `~/.openclaw/gateway.cmd`，让 Gateway 通过应用内置的 Electron Node runtime 运行，不再依赖机器额外安装系统 Node。
