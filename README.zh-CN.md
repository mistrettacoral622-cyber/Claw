...
SSH Intercom 会以非交互模式运行：先在被控制的机器上点击 **准备这台机器**，然后在控制端保存 SSH key 免密登录或可选 SSH 密码。如果暂时没有免密登录，也可以在路由里保存可选 SSH 密码；KTClaw 会把密码放在本机加密 secret store 中，并使用内置 SSH 客户端执行远端 OpenClaw 命令。第一阶段的 SFTP 上传/下载需要这份已保存的 SSH 密码。
